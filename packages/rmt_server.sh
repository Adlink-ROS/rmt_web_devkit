...
if [ "$1" = "stop" ]; then
    ...
    # Use port 9527
    sudo docker run -d -p 9527:9527 --rm --name rmt-frontend-container rmt-frontend
    # Use port 8080
    sudo docker run -d --privileged --volume /var/run/dbus:/var/run/dbus --network=host --rm --name rmt-backend-container rmt-backend
else
